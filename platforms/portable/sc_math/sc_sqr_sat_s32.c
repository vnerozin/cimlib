...
/*******************************************************************************
 * This function square input value, 32 bit signed, result with saturation
 * control.
 *
...
uint32_t sc_sqr_sat_s32(int32_t x, int radix)
{
    ...
    tmp = (uint64_t)(((int64_t)x * x) >> radix);
...
/*******************************************************************************
 * This function tests 'sc_sqr_sat_s32' function. Returns 'true' if validation
...
bool test_sc_sqr_sat_s32(void)
{
    ...
    /* Call 'sc_sqr_sat_s32' function */
    for (n = 0; n < 4; n++) {
        y[n] = sc_sqr_sat_s32(x[n], RADIX);
    }
