...
/*******************************************************************************
 * This function tests 'sc_mac_s16' function. Returns 'true' if validation
...
bool test_sc_mac_s16(void)
{
    ...
    /* Call 'sc_mac_s16' function */
    acc = sc_mac_s16(x[0], y[0], 0, RADIX);
    for(n = 1; n < 4; n++) {
        acc = sc_mac_s16(x[n], y[n], acc, RADIX);
    }
