...
/*******************************************************************************
 * This function tests 'sc_mac_c32' function. Returns 'true' if validation
...
bool test_sc_mac_c32(void)
{
    ...
    static cint32_t res = CONST_CPLX(2.6855964661E+00, 0);
    bool flOk = true;

    /* Call 'sc_mac_c32' function */
    for (n = 0; n < 4; n++) {
        acc = sc_mac_c32(x[n], y[n], acc, RADIX);
    }
