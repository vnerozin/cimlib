 ...
 * 16 bit unsigned. Returns zero if 'x' is equal zero.
 *
...
{
    /* TODO Optimise constant and table using 16 bit de Bruijn sequence */
...
/*******************************************************************************
 * This function tests 'sc_log2_int_u16' function. Returns 'true' if
...
bool test_sc_log2_int_u16(void)
{
    int n;
    int y[4];
    static uint16_t x[4] = {0, (1 << 10), 4321, 0xFFFF};
    static int res[4] = {0, 10, 12, 15};
    bool flOk = true;

    /* Call 'sc_log2_int_u16' function */
    for(n = 0; n < 4; n++) {
        y[n] = sc_log2_int_u16(x[n]);
    }
